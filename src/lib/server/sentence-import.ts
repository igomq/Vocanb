...
import { sentenceImportProvider } from './sentence-ai';
import { createSentenceBook } from './sentence-storage';
import { setItemFolder } from './folders';
import { fail, redirect } from '@sveltejs/kit';

const PDF_MAGIC = '%PDF-';

export async function importSentenceBook(request: Request, userId: string) {
	let data: FormData;
	try {
	...
	if (file.size > SENTENCE_PDF_MAX_BYTES) return fail(400, { message: 'PDF 파일이 너무 큽니다.' });
	...
	try {
		...
		const folderId = String(data.get('folderId') || '').trim() || null;
		...
		if (folderId) {
			try {
				await setItemFolder(userId, 'sentence', book.id, folderId);
			} catch (error) {
