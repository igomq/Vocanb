...
export function getVertexConfig() {
	...
	const model = (process.env.VERTEX_MODEL || env.VERTEX_MODEL)?.trim() || 'gemini-3.8-flash';
	if (location !== 'global') throw new Error('GOOGLE_CLOUD_LOCATION은 global이어야 합니다.');
	if (model !== 'gemini-3.8-flash') throw new Error('VERTEX_MODEL은 gemini-3.8-flash여야 합니다.');
